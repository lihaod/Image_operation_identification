...
public:
	...
	s1(QuantStep *quantStep, Config *config) : s(config)
	{
		this->CreateKernels();
		quantMultiplier = 1;
		

		for (int qIndex=0; qIndex < (int)quantStep->s1_spam14h.size(); qIndex++)
		{
			float q = quantStep->s1_spam14h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_spam14h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_spam14v.size(); qIndex++)
		{
			float q = quantStep->s1_spam14v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_spam14v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax22h.size(); qIndex++)
		{
			float q = quantStep->s1_minmax22h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax22h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax22v.size(); qIndex++)
		{
			float q = quantStep->s1_minmax22v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax22v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax24.size(); qIndex++)
		{
			float q = quantStep->s1_minmax24[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax24(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax34.size(); qIndex++)
		{
			float q = quantStep->s1_minmax34[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax34(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax34h.size(); qIndex++)
		{
			float q = quantStep->s1_minmax34h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax34h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax34v.size(); qIndex++)
		{
			float q = quantStep->s1_minmax34v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax34v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax41.size(); qIndex++)
		{
			float q = quantStep->s1_minmax41[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax41(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax48h.size(); qIndex++)
		{
			float q = quantStep->s1_minmax48h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax48h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax48v.size(); qIndex++)
		{
			float q = quantStep->s1_minmax48v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax48v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s1_minmax54.size(); qIndex++)
		{
			float q = quantStep->s1_minmax54[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s1_minmax54(q, config));
			}
			else
			{
				this->qs.push_back(q);

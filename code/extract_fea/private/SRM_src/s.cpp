...
mat2D<int>* s::Quantize(mat2D<int>* residual, float totalKernelQ)
{
	...
	for (int r=0; r<residual->rows; r++)
	{
		for (int c=0; c<residual->cols; c++)
		{
			...
			// rounding
			if (config->roundup5)
				tempValI = (tempValF>0.0) ? floor(tempValF + 0.5) : ceil(tempValF - 0.5);
			else
				tempValI = (tempValF - floor(tempValF) > 0.5) ? ceil(tempValF) : floor(tempValF);
...
bool s::findQuanStep(float queryQ, int &pos)
{
	for (int i = 0; i < this->qs.size(); i++)
	{
		if (std::abs(queryQ - this->qs[i]) < 0.00000001)
		{
			pos = i;
			return true;
		}

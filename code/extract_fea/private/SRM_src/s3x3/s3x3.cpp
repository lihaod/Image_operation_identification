...
public:
	...
	s3x3(QuantStep *quantStep, Config *config) : s(config)
	{
		this->CreateKernels();
		quantMultiplier = 4;

		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_spam11.size(); qIndex++)
		{
			float q = quantStep->s3x3_spam11[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_spam11(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_spam14h.size(); qIndex++)
		{
			float q = quantStep->s3x3_spam14h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_spam14h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_spam14v.size(); qIndex++)
		{
			float q = quantStep->s3x3_spam14v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_spam14v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_minmax22h.size(); qIndex++)
		{
			float q = quantStep->s3x3_minmax22h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_minmax22h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_minmax22v.size(); qIndex++)
		{
			float q = quantStep->s3x3_minmax22v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_minmax22v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_minmax24.size(); qIndex++)
		{
			float q = quantStep->s3x3_minmax24[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_minmax24(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s3x3_minmax41.size(); qIndex++)
		{
			float q = quantStep->s3x3_minmax41[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s3x3_minmax41(q, config));
			}
			else
			{
				this->qs.push_back(q);

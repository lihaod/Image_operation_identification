...
public:
	...
	s2(QuantStep *quantStep, Config *config) : s(config)
	{
		this->CreateKernels();
		quantMultiplier = 2;

		for (int qIndex = 0; qIndex < (int)quantStep->s2_spam12h.size(); qIndex++)
		{
			float q = quantStep->s2_spam12h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_spam12h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_spam12v.size(); qIndex++)
		{
			float q = quantStep->s2_spam12v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_spam12v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_minmax21.size(); qIndex++)
		{
			float q = quantStep->s2_minmax21[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_minmax21(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_minmax24h.size(); qIndex++)
		{
			float q = quantStep->s2_minmax24h[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_minmax24h(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_minmax24v.size(); qIndex++)
		{
			float q = quantStep->s2_minmax24v[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_minmax24v(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_minmax32.size(); qIndex++)
		{
			float q = quantStep->s2_minmax32[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_minmax32(q, config));
			}
			else
			{
				this->qs.push_back(q);
		...
		for (int qIndex = 0; qIndex < (int)quantStep->s2_minmax41.size(); qIndex++)
		{
			float q = quantStep->s2_minmax41[qIndex];
			int qPos = 0;
			if (findQuanStep(q, qPos))
			{
				this->submodels[qPos].push_back(new s2_minmax41(q, config));
			}
			else
			{
				this->qs.push_back(q);
